...
void work(unsigned char, unsigned char, unsigned char);
void servo1(unsigned char);
void servo2(unsigned char);
void servo3(unsigned char);
void servo4(unsigned char);
void delayms(unsigned int);
unsigned char glv1 = 80;
unsigned char glv2 = 70;
unsigned char glv3 = 100;
void main(void) {
    // led
    TRISBbits.TRISB1 = 0;
    PORTBbits.RB1 = 1;
    ...
    while(1){
        ...
        servo2(glv2);
        servo3(glv3);
        for(unsigned char i = 80; i < 105; i++){
            for(unsigned char j = 0; j < 3; j++){
                servo1(i);
            }
        }
        for(unsigned char i = 70; i < 85; i++){
            for(unsigned char j = 0; j < 3; j++){
                servo2(i);
            }
        }
        for(unsigned char i = 85; i > 80; i--){
            for(unsigned char j = 0; j < 3; j++){
                servo3(i);
            }
        }
        delayms(1200);
        for(unsigned char i = 85; i > 60; i--){
            for(unsigned char j = 0; j < 3; j++){
                servo2(i);
            }
        }
        for(unsigned char i = 105; i > 50; i--){
            for(unsigned char j = 0; j < 3; j++){
                servo1(i);
            }
        }
        ...
        delayms(1000);
        for(unsigned char i = 80; i > 65; i--){
            for(unsigned char j = 0; j < 3; j++){
                servo2(i);
            }
        }
        for(unsigned char i = 50; i < 80; i++){
            for(unsigned char j = 0; j < 3; j++){
                servo1(i);
            }
        }
